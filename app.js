...
(function() { // Wrap page logic in IIFE to avoid global scope pollution
    ...
    let firmwareLoaded = false; // Flag for standard firmware loading status
    const pageType = document.body?.dataset.page === 'bootloader' ? 'bootloader' : 'standard';
    const isBootloaderPage = pageType === 'bootloader';
    const firmwareLoadPromises = {};
    let flashCompletionHandled = false;
    ...
    const storagePrefix = isBootloaderPage ? 'ucritterBootloader' : 'ucritterStandard';
    const stateKey = `${storagePrefix}FlashState`;
    const serialKey = `${storagePrefix}FlashSerial`;
    const blStepKey = `${storagePrefix}WizardStep`;
    ...
    function saveState(stateToSave, deviceSerial = serial) {
        ...
        if (stateToSave !== STATE.FLASH_COMPLETE) {
            flashCompletionHandled = false;
        }
    ...
    function clearState() {
        ...
        hideTimeSetButton();
        console.log(`State cleared (was ${previousState})`); if (dfuUtil) dfuUtil.clearLog(); updateUI();
    }

    function isFirmwareReady(name = 'default') {
        return !!dfuUtil?.getFirmwareFile(name);
    }

    async function ensureFirmwareLoaded(name = 'default') {
        if (isFirmwareReady(name)) {
            return true;
        }
        const pendingLoad = firmwareLoadPromises[name];
        if (!pendingLoad) {
            return false;
        }
        try {
            ...
            console.error(`Firmware '${name}' failed to load.`, error);
        }
        return isFirmwareReady(name);
    }

    function loadFirmwareAsset(url, name, onReady, onError) {
        if (!dfuUtil) {
            return Promise.reject(new Error("DFU utility not initialized."));
        }
        if (!firmwareLoadPromises[name]) {
            firmwareLoadPromises[name] = dfuUtil.loadFirmware(url, name);
        }
        return firmwareLoadPromises[name]
            .then((result) => {
                if (name === 'default') firmwareLoaded = true;
                if (name === 'bootloader') blFirmwareLoaded = true;
                if (typeof onReady === 'function') onReady(result);
                return result;
            })
            .catch((error) => {
                if (name === 'default') firmwareLoaded = false;
                if (name === 'bootloader') blFirmwareLoaded = false;
    ...
    function updateUI() {
        ...
        switch (currentState) {
            case STATE.IDLE: updateStatus("Ready to connect", "info"); buttonText = firmwareLoaded ? "Start Standard Update" : "Loading Firmware..."; buttonDisabled = !firmwareLoaded; break;
            ...
            case STATE.FLASH_COMPLETE:
                ...
                if (!flashCompletionHandled) {
                    flashCompletionHandled = true;
                    if (flashContext.onComplete) {
                        setTimeout(() => {
                            flashContext.onComplete();
                            flashContext = { firmwareName: 'default', onComplete: null };
                        }, 3000);
                    } else {
    ...
    async function runFlashWorkflow() {
          ...
          const firmwareName = flashContext.firmwareName || 'default';
          if (!await ensureFirmwareLoaded(firmwareName)) { handleError(new Error("FW missing."), "Firmware missing!"); return; }
          const firmware = dfuUtil.getFirmwareFile(firmwareName); if (!firmware) { handleError(new Error("FW missing."), "Firmware missing!"); return; }
    ...
    async function handleConnectClick() {
        ...
        const requiredFirmware = flashContext.firmwareName || 'default';
        if (currentState === STATE.IDLE && !await ensureFirmwareLoaded(requiredFirmware)) { updateStatus("FW loading...", "info"); return; } if (!dfuUtil) { handleError(new Error("DFU Util missing."), "Init error."); return; }
    ...
    function advanceBootloaderStep(step, restoring = false) {
        const numericStep = step === 'done' ? 4 : parseInt(step, 10);
        bootloaderStep = Number.isFinite(numericStep) ? numericStep : 1;
        if (!restoring) {
            if (step === 'done' || bootloaderStep > 3) {
                sessionStorage.removeItem(blStepKey);
            } else {
                sessionStorage.setItem(blStepKey, String(bootloaderStep));
            }
        }
        ...
        document.querySelectorAll('.bl-step').forEach(el => {
            ...
            if (s < bootloaderStep) el.classList.add('completed');
            else if (s === bootloaderStep) el.classList.add('active');
        });
        ...
        if (bootloaderStep === 3) {
            ...
            if (status3) {
                status3.textContent = firmwareLoaded ? 'Ready' : 'Loading standard firmware...';
                status3.className = 'status status-info';
            }
        }
        ...
        if (step === 'done' || bootloaderStep > 3) {
            ...
            if (blLog && !blLog.querySelector('[data-bl-complete="true"]')) {
                let p = document.createElement('p');
                p.className = 'success';
                p.dataset.blComplete = 'true';
    ...
    function loadStandardFirmware() {
        return loadFirmwareAsset(
            "zephyr.signed.bin",
            "default",
            () => {
                if (dfuUtil) dfuUtil.logInfo("Standard firmware loaded.");
                updateUI();
                if (bootloaderStep === 3 && currentState === STATE.IDLE) {
                    advanceBootloaderStep(3, true);
                }
            },
            (err) => {
                const status3 = document.getElementById('bl-status-step3');
                if (status3 && isBootloaderPage) {
                    status3.textContent = 'Standard firmware not available';
                    status3.className = 'status status-error';
                }
                if (!isBootloaderPage) {
                    handleError(err, `FW load failed: ${err.message}. Refresh.`);
                } else if (dfuUtil) {
                    dfuUtil.logError(`Standard firmware load failed: ${err.message}`);
                }
            }
        );
    }

    function loadBootloaderFirmware() {
        return loadFirmwareAsset(
            "zephyr-bl-update.signed.bin",
            "bootloader",
            () => {
                ...
                if (status1) {
                    ...
                    status1.className = 'status status-info';
                }
                if (dfuUtil) dfuUtil.logInfo("Bootloader firmware loaded.");
            },
            (err) => {
                const status1 = document.getElementById('bl-status-step1');
                if (status1) {
                    status1.textContent = 'Firmware not available';
                    status1.className = 'status status-error';
                }
                if (dfuUtil) dfuUtil.logError(`Bootloader firmware load failed: ${err.message}`);
            }
        );
    }

    function restoreBootloaderFlashContext(bootloaderLog) {
        const savedStep = parseInt(sessionStorage.getItem(blStepKey) || '1', 10);
        advanceBootloaderStep(savedStep >= 1 && savedStep <= 3 ? savedStep : 1, true);
        ...
        if (bootloaderLog && dfuUtil) {
            dfuUtil.setLogContext(bootloaderLog);
        }
    }

    function setupInstructionToggle(instructionsColumn) {
        const osButtons = document.querySelectorAll('.os-btn');
        const instructionsSections = instructionsColumn ? instructionsColumn.querySelectorAll('section.instructions[data-ins]') : [];
        if (!instructionsColumn || !instructionsSections.length || !osButtons.length) {
            return;
        }
    ...
    function setupSerialTriggerButton() {
        ...
        serialDfuBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                setTimeout(() => {
                    if (serialDfuStatusSpan) {
                        serialDfuStatusSpan.textContent = 'Bootloader mode ready. Click "Start Standard Update" above when you are ready.';
                    }
    ...
    function setupBootloaderWizard(bootloaderLog) {
        const blBtn1 = document.getElementById('bl-connect-step1');
        ...
        if (blBtn1) {
            blBtn1.addEventListener('click', () => {
                ...
                advanceBootloaderStep(1);
                if (bootloaderLog && dfuUtil) dfuUtil.setLogContext(bootloaderLog);
                clearState();
                flashContext = {
                    firmwareName: 'bootloader',
                    onComplete: () => {
                ...
                handleConnectClick._skipDfuTrigger = true;
                handleConnectClick();
            });
        }
        ...
        if (blBtn3) {
            blBtn3.addEventListener('click', async () => {
                if (!await ensureFirmwareLoaded('default')) {
                    if (status3) {
                        status3.textContent = 'Standard firmware is still loading. Try again in a moment.';
                        status3.className = 'status status-error';
                    }
                    return;
                }
                ...
                if (webSerial.supported) {
                    if (status3) {
                        status3.textContent = 'Select your device\'s serial port...';
                        status3.className = 'status status-info';
                    ...
                    try {
                        await webSerial.triggerDfu();
                        if (status3) {
                            status3.textContent = 'DFU command sent! Waiting for reboot...';
                            status3.className = 'status status-success';
                        }
                        await new Promise(r => setTimeout(r, 4000));
                    } catch (e) {
                        if (status3) {
                            status3.textContent = e.name === 'NotFoundError'
                                ? 'No port selected. Try again or enter bootloader manually.'
                                : `Serial error: ${e.message}. Try entering bootloader manually.`;
                            status3.className = 'status status-error';
                        }
    ...
    function attachTimeSetHandler(button, status) {
        if (!button) {
            return;
        }
        ...
        button.addEventListener('click', async () => {
            ...
            try {
                ...
                if (status) {
                    status.textContent = `Clock set to ${timeStr}`;
                    status.className = 'status status-success';
                }
                if (dfuUtil) dfuUtil.logSuccess(`Device time set to ${timeStr}`);
            } catch (e) {
                if (status) {
                    status.textContent = e.name === 'NotFoundError' ? 'No device selected.' : `Error: ${e.message}`;
    ...
    function initializePage() {
         console.log(`Initializing μCritter Pupdate Page (${pageType})...`);
         ...
         const bootloaderLog = document.getElementById("bootloaderLog");
         const isWebUsbSupported = typeof navigator.usb !== 'undefined';

         if (!isWebUsbSupported) {
             console.warn("WebUSB not supported.");
             if (webUsbNotice) {
                 webUsbNotice.innerHTML = `<p><strong>WebUSB not supported.</strong> Use Chrome or Chromium.</p>`;
         ...
         const initialLog = isBootloaderPage ? bootloaderLog : downloadLog;
         try {
             if (initialLog) {
                 dfuUtil.setLogContext(initialLog);
                 console.log("Log context set.");
             } else {
                 throw new Error("Log element missing.");
             }
        ...
        dfuUtil.setOnDisconnectCallback((reason) => {
            ...
            if (![STATE.IDLE, STATE.ERROR, STATE.FLASH_COMPLETE, STATE.FLASHING, STATE.WAITING_DISCONNECT, STATE.PROMPT_REFRESH_1, STATE.PROMPT_REFRESH_2, STATE.PROMPT_CONNECT_STAGE2, STATE.PROMPT_CONNECT_FLASH].includes(currentState)) {
                if (currentDevice) {
                    handleError(new Error("Device disconnected."), "Device Disconnected!");
                    currentDevice = null;
                } else {
                    dfuUtil.logWarning("Disconnect event for cleared ref.");
                }
            } else if (currentState === STATE.WAITING_DISCONNECT) {
                dfuUtil.logInfo("Disconnected as expected.");
                currentDevice = null;
            } else {
                dfuUtil.logInfo(`Disconnected in state: ${currentState}`);
                currentDevice = null;
            }
            updateUI();
        });
        ...
        if (isBootloaderPage) {
            restoreBootloaderFlashContext(bootloaderLog);
            loadBootloaderFirmware().catch(() => {});
            loadStandardFirmware().catch(() => {});
            setupBootloaderWizard(bootloaderLog);
        } else {
            loadStandardFirmware().catch(() => {});
            loadAndDisplayVersionInfo();
            if (!connectButton) {
                handleError(new Error("Connect button missing."), "Init Error: Connect button missing.");
                return;
            }
            connectButton.addEventListener('click', handleConnectClick);
            setupInstructionToggle(instructionsColumn);
            setupSerialTriggerButton();
        }

        attachTimeSetHandler(document.getElementById('timeSetBtn'), document.getElementById('timeSetStatus'));
        ...
        attachTimeSetHandler(standaloneTimeSetBtn, standaloneTimeSetStatus);
        ...
        if (typeof runAutoConnectSequence === "function") {
            ...
            console.error("runAutoConnectSequence missing!");
        }
    }
