...
(function() { // Wrap page logic in IIFE to avoid global scope pollution
    ...
    async function loadState() {
        ...
        if (savedState && Object.values(STATE).includes(savedState) &&
            savedState !== STATE.IDLE && savedState !== STATE.FLASH_COMPLETE)
        {
            requiresSerial = [
            ...
            if (requiresSerial && !savedSerial) {
             ...
             stateIsValid = true;
             requiresSerial = true; // Assume error state might have associated serial
    ...
    function clearState() {
        ...
        if (dfuUtil && downloadLog) dfuUtil.clearLog(downloadLog);
        updateUI();
    }
    ...
    function updateStatus(message, type = 'info') {
        ...
        statusDisplay.classList.add('status', `status-${type}`);
        if (dfuUtil) { console.log(`Status Update (${type}): ${message}`); }
    }

    function updateUI() {
        if (!connectButton) { return; }
        if (dfuUtil) { console.log("Updating UI for state:", currentState); }
        ...
        switch (currentState) {
            case STATE.IDLE:
                ...
                break;
            case STATE.CONNECTING_STAGE1: // Fallthrough
            case STATE.CONNECTING_STAGE2: // Fallthrough
            case STATE.CONNECTING_FLASH:
                updateStatus(`Connecting (${currentState})... Check browser pop-up!`, "info");
                buttonText = "Connecting..."; buttonDisabled = true; break;
            case STATE.WAITING_DISCONNECT:
                updateStatus("Stage 1 Connected. Switching device mode...", "info");
                buttonText = "Switching..."; buttonDisabled = true;
                if (dfuUtil) dfuUtil.logInfo("Attempting detach..."); break;
            case STATE.PROMPT_REFRESH_1:
                updateStatus("Stage 1 Done! Please REFRESH PAGE NOW (Ctrl+R or Cmd+R).", "prompt");
                buttonText = "REFRESH PAGE NOW"; buttonDisabled = true;
                if (dfuUtil) dfuUtil.logSuccess("Ready for first refresh. Please refresh the page."); break;
            case STATE.PROMPT_CONNECT_STAGE2:
                 updateStatus("Click button to grant permission for Stage 2.", "prompt");
                 buttonText = "Connect Stage 2"; buttonDisabled = false;
                 if (dfuUtil) dfuUtil.logWarning("Browser needs permission for Stage 2. Please click 'Connect Stage 2'."); break;
            case STATE.PROMPT_CONNECT_FLASH:
                 updateStatus("Click button to grant permission for Flash.", "prompt");
                 buttonText = "Connect to Flash"; buttonDisabled = false;
                 if (dfuUtil) dfuUtil.logWarning("Browser needs final permission. Please click 'Connect to Flash'."); break;
            case STATE.WAITING_STABLE:
                  updateStatus("Stage 2 Connected. Stabilizing...", "info");
                  buttonText = "Stabilizing..."; buttonDisabled = true;
                  if (dfuUtil) dfuUtil.logInfo("Waiting briefly for device to stabilize..."); break;
            case STATE.PROMPT_REFRESH_2:
                updateStatus("Stage 2 Ready! Please REFRESH PAGE AGAIN.", "prompt");
                buttonText = "REFRESH PAGE AGAIN"; buttonDisabled = true;
                if (dfuUtil) dfuUtil.logSuccess("Ready for final refresh. Please refresh the page."); break;
            case STATE.FLASHING:
                updateStatus("Flashing Firmware... Do not disconnect!", "info");
                buttonText = "Flashing..."; buttonDisabled = true;
                 if (dfuUtil) dfuUtil.logInfo("Starting firmware flash process..."); break;
            case STATE.FLASH_COMPLETE:
                updateStatus("Pupdate Complete! Critter rebooting.", "success");
                buttonText = "Done!"; buttonDisabled = true;
                if (dfuUtil) dfuUtil.logSuccess("Firmware flashed successfully! Your Critter should reboot now.");
                 setTimeout(clearState, 6000); break;
            case STATE.ERROR:
                buttonText = "Error Occurred - Reset?"; buttonDisabled = false; break;
            default:
                 updateStatus("Unknown application state", "error");
                 buttonText = "Error"; buttonDisabled = true;
        }
    ...
    function handleError(error, userMsg = "An error occurred. Check console.") {
        ...
        if (currentState !== STATE.ERROR) { saveState(STATE.ERROR); }
    }
    ...
    async function attemptConnection(attemptVid, attemptSerial, allowRequestPrompt = false) {
          connectAttempts++;
          if (connectAttempts > MAX_CONNECT_ATTEMPTS) { throw new Error(`Maximum connection attempts (${MAX_CONNECT_ATTEMPTS}) reached.`); }
          ...
          if (attemptSerial) { permittedDeviceFilter[0].serialNumber = attemptSerial; }
          try {
              const devices = await navigator.usb.getDevices();
              ...
              if (matchingDevice) {
                  ...
                  connectAttempts = 0; dfuUtil.logSuccess(`Connected to permitted device.`); currentDevice = connectedDfuDevice; return connectedDfuDevice;
          ...
          dfuUtil.logInfo(`Requesting device permission from user with filter: ${JSON.stringify(promptFilter)}`);
           try {
              ...
              vid = selectedUsbDevice.vendorId; serial = selectedUsbDevice.serialNumber || ''; sessionStorage.setItem(serialKey, serial);
              ...
              connectAttempts = 0; dfuUtil.logSuccess(`Connected to user-selected device.`); currentDevice = connectedDfuDevice; return connectedDfuDevice;
           } catch(error) {
               if (error.name === 'NotFoundError') { if (promptFilter && promptFilter.length > 0) { dfuUtil.logError("Device selection prompt failed: No matching device found by Chrome, even though OS might see it."); throw new Error("No matching device found by Chrome. Check OS/driver or connection."); } else { throw new Error("No device selected by the user."); } }
    ...
    async function handleConnectClick() {
        ...
        if (currentState === STATE.IDLE && !firmwareLoaded) { updateStatus("Firmware is still loading, please wait...", "info"); return; }
        if (!dfuUtil) { handleError(new Error("DFU Utility not ready."), "Initialization error, please refresh."); return; }
        ...
        if (currentState === STATE.IDLE) {
             ...
             try {
                 ...
                 if (!currentDevice || !currentDevice.device_ || currentDevice.device_.productName !== 'MCUBOOT') { throw new Error(`Incorrect device connected: ${currentDevice?.device_?.productName || 'Unknown'}. Expected 'MCUBOOT'.`); }
                 dfuUtil.logSuccess(`Connected to ${currentDevice.device_.productName} (Stage 1). Serial: ${currentDevice.device_.serialNumber || 'N/A'}`); serial = currentDevice.device_.serialNumber || '';
                 dfuUtil.logInfo("Waiting briefly before detaching for mode switch..."); await sleep(300);
                 ...
                 await Promise.race([ currentDevice.waitDisconnected(5000), new Promise((_, reject) => setTimeout(() => reject(new Error("Device did not disconnect within 5 seconds after detach.")), 5000)) ]);
              ...
              try {
                  dfuUtil.logInfo("Waiting briefly before requesting device (Stage 2)..."); await sleep(500);
                  ...
                  try {
                      dfuUtil.logInfo("Checking DFU status..."); let status = await currentDevice.getStatus(); dfuUtil.logInfo(`DFU Status: State=${status.state}, Status=${status.status}`);
              ...
              try {
                   dfuUtil.logInfo("Waiting briefly before requesting device (Flash)..."); await sleep(500);
                  await attemptConnection(vid, serial, true);
     ...
     async function runAutoConnectSequence() {
          ...
          if (currentState === STATE.PROMPT_REFRESH_1) {
               dfuUtil.logInfo("Detected state PROMPT_REFRESH_1. Attempting automatic Stage 2 connection..."); saveState(STATE.CONNECTING_STAGE2, serial);
               try {
                   dfuUtil.logInfo("Waiting briefly before auto-connecting (Stage 2)..."); await sleep(500);
                    await attemptConnection(vid, serial, false);
                    dfuUtil.logSuccess(`Auto-reconnected to ${currentDevice.device_.productName} (Stage 2). Serial: ${serial || 'N/A'}`); saveState(STATE.WAITING_STABLE, serial); await sleep(1500);
                    try {
                        dfuUtil.logInfo("Checking DFU status after auto-reconnect..."); let status = await currentDevice.getStatus(); dfuUtil.logInfo(`DFU Status: State=${status.state}, Status=${status.status}`);
                    ...
                    saveState(STATE.PROMPT_REFRESH_2, serial);
               } catch (error) { if (error instanceof NeedsUserGestureError) { dfuUtil.logWarning("Permissions needed for Stage 2 (not persisted). Asking user to click."); saveState(STATE.PROMPT_CONNECT_STAGE2, serial); } else { handleError(error, `Automatic connection failed (Stage 2): ${error.message || error}`); } }
           }
           else if (currentState === STATE.PROMPT_REFRESH_2) {
                dfuUtil.logInfo("Detected state PROMPT_REFRESH_2. Attempting automatic Flash connection..."); saveState(STATE.CONNECTING_FLASH, serial);
                try {
                    dfuUtil.logInfo("Waiting briefly before auto-connecting (Flash)..."); await sleep(500);
                     await attemptConnection(vid, serial, false);
                     dfuUtil.logSuccess(`Auto-reconnected to ${currentDevice.device_.productName} (Ready to Flash!). Serial: ${serial || 'N/A'}`); saveState(STATE.FLASHING, serial); await runFlashWorkflow();
                } catch (error) { if (error instanceof NeedsUserGestureError) { dfuUtil.logWarning("Permissions needed for Flash (not persisted). Asking user to click."); saveState(STATE.PROMPT_CONNECT_FLASH, serial); } else { handleError(error, `Automatic connection failed (Final Flash): ${error.message || error}`); } }
           } else { console.log("No automatic connection action needed for current state:", currentState); updateUI(); }
     }
    ...
    async function initializePage() {
         ...
         if (!isWebUsbSupported) {
          ...
          // Make sure the global dfuUtil (from dfu-util.js) exists before proceeding
         if (typeof window.dfuUtil === 'undefined') {
             handleError(new Error("DFU Utility object not found."), "Initialization Failed: DFU utility missing.");
             return; // Stop initialization if dfuUtil isn't ready
         }
         dfuUtil = window.dfuUtil; // Assign global dfuUtil to local variable

         // Ensure the core DFU library object exists too
         if (typeof dfu === 'undefined') {
              handleError(new Error("Core DFU library (dfu.js) not found."), "Initialization Failed: Core DFU library missing.");
              return; // Stop initialization if dfu isn't ready
         }
         ...
         try {
             ...
             // Call init explicitly now that we know dfuUtil exists
             dfuUtil.init();
             console.log("dfuUtil.init() called successfully.");
